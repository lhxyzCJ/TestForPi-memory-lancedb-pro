/**
 * memory-lancedb-pro — pi coding agent extension entry point.
 *
 * Load this extension from:
 *   - ~/.pi/agent/extensions/            (global, all projects)
 *   - .pi/extensions/                    (project-local)
 *   - pi -e ./pi-adapter/index.ts        (quick test)
 *
 * or install as a pi package (see package.json "pi" field).
 *
 * The OpenClaw plugin core registers unchanged through the API shim in
 * shim.ts; this file is the only pi-specific bootstrap.
 */
...
export default function (pi: ExtensionAPI): void {
  ...
  if (pluginConfig === undefined) {
    console.warn(
      "[memory-lancedb-pro] no config file found; memory extension disabled.\n" +
