...
import { createRetriever, normalizeRetrievalConfig, type RetrievalConfig } from "../src/retriever.js";
...
const resolvePathImpl = (value: string): string => expandHomeDir(expandEnvVars(value.trim()));

function resolveSecret(value: unknown, label: string): string {
  if (typeof value === "string" && value.trim().length > 0) return expandEnvVars(value.trim());
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (source === "env") {
      ...
      if (!v) throw new Error(`environment variable ${id} is not set`);
      return v;
    }
    if (source === "file") {
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (pluginConfig === undefined) {
    console.error(
      "[memory-lancedb-pro] no config file found.\n" +
  ...
  const retrievalConfig = normalizeRetrievalConfig(
    config.retrieval as Parameters<typeof normalizeRetrievalConfig>[0],
  ) as RetrievalConfig;
