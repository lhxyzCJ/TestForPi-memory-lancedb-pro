  ...
  register(api: OpenClawPluginApi) {
    ...
    if (config.autoRecall === true && recallMode !== "off") {
      ...
      api.on("message_received", (event: any, ctx: any) => {
        ...
        if (text) pushRecentUserText(cacheKey, text);
      });
      ...
      const pushRecentUserText = (key: string, text: string) => {
        const ring = recentUserTexts.get(key) ?? [];
        ring.push(text);
        if (ring.length > RECENT_RING_MAX) ring.shift();
        recentUserTexts.set(key, ring);
      };
      /** 保守跳过:query 与最近 RECENT_RING_MAX 条用户消息互为子串才跳(话题已在当前上下文)。
       *  仅子串级,不做语义判断——避免"重复话题但记忆已更新"的误伤。 */
      const shouldSkipByRecentContext = (query: string, key: string) => {
        const ring = recentUserTexts.get(key) ?? [];
        ...
        for (const m of event?.messages ?? []) {
          if (m?.role === "assistant" && Array.isArray(m.content)) {
            const text = m.content
              ...
              .join("\n")
              .trim();
            if (text) lastAssistantReply.set(cacheKey, text);
            break;
          }
        }
      });
      ...
      api.on("before_prompt_build", async (event: any, ctx: any) => {
        ...
        const recallWork = async (): Promise<{ prependContext: string; ephemeral?: boolean } | undefined> => {
          ...
          if (shouldSkipByRecentContext(recallQuery, cacheKey)) return;
          const lastReply = lastAssistantReply.get(cacheKey);
          if (lastReply) recallQuery = `${recallQuery}\n${lastReply}`;
          ...
          if (smartExtractor) {
            ...
            if (
              ...
              if (persistedSomething || !admittedOnlyByExplicitRemember) {
                ...
                const remainingDeferred = (autoCaptureDeferredFlushTexts.get(sessionKey) || [])
                  .filter((text) => !consumedTexts.has(text));
                if (remainingDeferred.length === 0) {
                  autoCaptureDeferredFlushTexts.delete(sessionKey);
                } else {
                  autoCaptureDeferredFlushTexts.set(sessionKey, remainingDeferred);
                }
              }
