...
## Требования к среде выполнения (pi)

Следующие требования исходят от хоста pi и не настраиваются в `memory-lancedb-pro.json5`:

1. **Провайдер по умолчанию** — Встроенные субагенты рефлексии/снов запускаются через CLI `pi` без `--provider`/`--model` и наследуют значение по умолчанию хоста. Укажите `defaultProvider`/`defaultModel` в `~/.pi/agent/settings.json` (например, `"defaultProvider": "opencode-go", "defaultModel": "deepseek-v4-flash"`), иначе субагенты могут откатиться на сломанного или непреднамеренного провайдера.
2. **Точка входа эмбеддингов** — API эмбеддингов должно быть доступно (например, запустите Ollama перед сеансом); недоступная точка входа молча понижает умное извлечение до regex-запасного варианта.
3. **Ключ API LLM** — Для умного извлечения требуется действительный ключ LLM (`llm.apiKey`, например, `"${OPENCODE_API_KEY}"`); без него извлечение откатывается к regex-захвату.
4. **Путь обновления** — pi загружает расширение по абсолютному пути, записанному в `settings.json` (`extensions`). После обновления этого репозитория повторно выполните `pi install` (или замените установленную копию), иначе pi продолжит загружать старую сборку.

**Известный разрыв с апстримом:** режим пакетной утилиты допуска апстрима (#941, `utilityMode: "batch"`, `utilityVetoThreshold`) не перенесён; `utilityMode` поддерживает только `"standalone" | "off"`.
